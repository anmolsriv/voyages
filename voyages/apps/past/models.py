...
class StanfordKin(models.Model):
    id_string = models.CharField(max_length=255, null=False)
    name = models.CharField(max_length=255, null=False)
    birth_date = models.DateField(null=False, blank=True)
    death_date = models.DateField(null=False, blank=True)


class EnslaverIdentity(EnslaverInfoAbstractBase):
    stanford_kin = models.ManyToManyField(StanfordKin, through='EnslaverIdentityStanfordKinConnection', related_name='+')
    class Meta:
        verbose_name = 'Enslaver unique identity and personal info'


class EnslaverEnslaverConnection(models.Model):
    source = models.ForeignKey(EnslaverIdentity, related_name="source", on_delete=models.CASCADE)
    target = models.ForeignKey(EnslaverIdentity, related_name="target", on_delete=models.CASCADE)
...
class EnslaverIdentityStanfordKinConnection(models.Model):
    enslaver_id = models.ForeignKey(EnslaverIdentity, on_delete=models.CASCADE)
    stanfordkin_id = models.ForeignKey(StanfordKin, on_delete=models.CASCADE)
...
class Enslaved(models.Model):
    ...
    voyage = models.ForeignKey(Voyage, related_name="enslaved_voyage_id", null=False)
...
class EnslavedSearch:
    ...
    def execute(self, fields):
        ...
        if self.year_range:
            ...
            q = q.filter(
                voyage__voyage_dates__imp_arrival_at_port_of_dis__range=[',,' + str(y) for y in self.year_range])
        ...
        if isinstance(self.order_by, list):
            ...
            for x in self.order_by:
                ...
                def add_names_sorting(sorted_name_fields, col_name, q):
                    ...
                    expressions = [Coalesce(F(name_field), fallback_name_val, output_field=CharField()) for name_field
                                   in sorted_name_fields]
